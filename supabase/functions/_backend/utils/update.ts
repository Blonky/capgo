...
export async function updateWithPG(
  ...
  const requestedInto = await requestInfosPostgres(c, platform, app_id, device_id, defaultChannel, drizzleClient, channelDeviceCount, manifestBundleCount, rolloutChannelCount, rolloutPausedVersionNames, version_name, needsMetadata)
  ...
  await Promise.all([
    createStatsVersion(c, version.name, app_id, 'get', channelData.channels.name),
